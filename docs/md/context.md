...
| 序 | 段 | 生成者 | 内容 | 条件 / 刷新 |
|---|---|---|---|---|
| 1 | 人设段 | `buildSystemBase`（`service/agent_service.go`） | 「你是 ezharness——…」+ 设置页 `SystemExtra` | session 创建时组装一次 |
| 2 | `<workspace>` | 同上 | 目录架构与读写权限 + `<@toolArg>` / `<$supper_url>` 语法 + `term_*` / `browser_*` 指引 | 同上 |
| 3 | `<memory>` | 同上 | 记忆结构说明 + `harness.md` 全文（`EnsureHarnessMd`） | 同上 |
| 4 | `<skills>` | 同上 | 技能清单（`skill.LoadDir`，按 `DisabledSkills` 过滤） | 有技能才有 |
| 5 | `<mcp>` | 同上 | 启用的 MCP server 清单 | 有 server 才有 |
| 6 | `<session>` 身份块 | `SysPrompt.identityFn`（`hooks/sysprompt.go` `SessionIdentityBlock`） | 会话 ID + 存档绝对路径（trim 折叠后的回忆入口） | **每次 render 实时求值**（换代后 ID 自动正确） |
| 7 | `<compact-summary>` | `hooks/archive.go` 归档时写入 summary 段 | 「上一会话已归档，原始记录在 …；本会话开始前的摘要：…」 | 仅 compact 换代后的 session |
...
# …/data/workspace          工作目录，草稿/脚本/命令产物放这里，自由读写（terminal 默认执行目录：…/data/workspace）
# …/data/workspace/tmp      用户上传附件的暂存目录；需要附件内容时用 read_file 按路径读取…
# …/data/memory/longterm    长期记忆，可写：harness.md 是索引（已注入上下文）…
# …/data/apps               快应用目录，由 save_app 工具写入，一般不手动改
# 规则：terminal 每条命令是独立进程（cd 不跨命令保留）；所有文件读写与命令一律绝对路径…
...
<compact-summary>
上一会话已归档，原始记录在 …/sessions/20260915-233831-6c845611/session.json（session.json 可读取全文）。本会话开始前的摘要：
（上一世代摘要正文）
</compact-summary>                                                       ← ⑦
# 系统环境
...
| ⑩ | `<context_trim kept="N">` | `hooks/trim.go` `doTrim` | 整理时尾插（序列尾部，时间序自然） | 水位自动（OnLoop）或模型调 `trim_context` | `session.trim` note / 标签 → note 分割线 |
