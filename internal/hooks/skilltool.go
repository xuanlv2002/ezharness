/*
skilltool 是 skill 的第三层加载：模型用 load_skill 工具按名称取
SKILL.md 全文 + 技能目录结构。三层分工：
  1. 名称与描述进 systemPrompt（session 创建时，见 agent_service
     buildSystemBase）——模型知道有什么可用；
  2. 本工具返回完整指令与文件路径——模型知道怎么用；
  3. 模型按返回的路径用 bash / read_file 调用 scripts 等子资源。
...
/* SkillTool 是按名称加载技能全文的工具注册名。 */
const SkillTool = "load_skill"
...
func (h *SkillToolHook) OnToolStart(ctx context.Context, state *types.LoopState, call *types.ToolCall) (ezhook.Action, error) {
	if call.Name != SkillTool {
...
func (h *SkillToolHook) render(ctx context.Context, s skill.Skill) string {
...
func (h *SkillToolHook) dirTree(ctx context.Context, skillPath string) string {
	dir := skillPath
...
func (skillToolShell) Name() string { return SkillTool }
